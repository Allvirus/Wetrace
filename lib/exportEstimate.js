...
// 连接池优化后的默认吞吐（条/秒）；旧版约 520，实测常可达数千～数万
const DEFAULT_MESSAGES_PER_SEC = 4000;
const MIN_MESSAGES_PER_SEC = 200;
const MAX_MESSAGES_PER_SEC = 80000;
...
function estimateExportDuration({
  ...
  const initSec = 3 * mult;
  const convOverhead = conversationCount * 0.08 * mult;

  let messagesPerSec = learned?.messagesPerSec ?? DEFAULT_MESSAGES_PER_SEC / mult;
  messagesPerSec = Math.max(MIN_MESSAGES_PER_SEC, Math.min(MAX_MESSAGES_PER_SEC, messagesPerSec));
...
function recordExportSample(previous, sample) {
  ...
  if (!durationSec || durationSec < 1 || messageCount <= 0) {
