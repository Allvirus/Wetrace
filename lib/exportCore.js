...
const { createMessageDbPool, queryTableRows } = require('./messageDbPool');
  ...
  try {
    for (const { name: table } of listMsgTables(db)) {
      ...
      const result = db.exec(
        `SELECT count(*) AS c, sum(CASE WHEN local_type = 34 THEN 1 ELSE 0 END) AS v, min(create_time) AS min_t, max(create_time) AS max_t FROM "${table}"`
      );
      ...
      if (minTime > 0 || maxTime > 0) {
        ...
        if (!existing) {
          timestampRanges.set(username, {
            first: minTime > 0 ? minTime : maxTime,
            last: maxTime > 0 ? maxTime : minTime,
          });
        } else {
          if (minTime > 0) existing.first = Math.min(existing.first, minTime);
          if (maxTime > 0) existing.last = Math.max(existing.last, maxTime);
        }
      }
    }
  }
...
function getTimeBoundsFromPool(pool, username) {
  ...
  for (const dbPath of dbPaths) {
    const db = pool.getDb(dbPath);
    const result = db.exec(
      `SELECT min(create_time) AS min_t, max(create_time) AS max_t FROM "${table}"`
    );
    const values = result[0]?.values?.[0];
    if (!values) continue;
...
async function getConversationTimeBounds({ wxDir, username }) {
  const resolvedWxDir = resolveWxDir(wxDir);
  const decryptedDir = resolveDecryptedDir(resolvedWxDir);
  const msgDbs = getMessageDbs(decryptedDir);
  if (msgDbs.length === 0) {
...
async function countConversationMessagesInRange({ wxDir, username, startTime, endTime }) {
  const resolvedWxDir = resolveWxDir(wxDir);
  const decryptedDir = resolveDecryptedDir(resolvedWxDir);
  const msgDbs = getMessageDbs(decryptedDir);
  if (msgDbs.length === 0) {
...
async function exportWeChatChats({
  ...
  const { writeChatFormats, createCsvWriter, writeHtmlIndex } = require('./exportFormats');
  const {
  ...
  const csvWriter = normalizedFormats.includes('csv')
    ? createCsvWriter(path.join(outputDir, 'messages.csv'))
    : null;
