const ERROR_RULES = [
  ...
  },
  {
    code: 'WTR-E003',
    match: (msg) =>
      /Hook 安装|install hook|Failed to open target process|Allocate trampoline|remote hook|SeDebugPrivilege|admin rights|administrator/i.test(
        msg
      ),
    title: 'Hook 安装失败',
    userMessage:
      '无法向微信进程安装 Hook。\n\n' +
      '常见原因：杀毒/安全软件拦截、微信版本不兼容、微信未在 Hook 就绪前保持未登录状态。\n' +
      '请关闭杀毒后重试；扫描时等「Hook 已就绪」再点击「登录」。',
    suggestions: ['暂时关闭杀毒/安全软件', 'Hook 就绪后再点登录', '确认微信为官方 PC 版'],
  },
  {
    code: 'WTR-E004',
    match: (msg) =>
    ...
    userMessage:
      '未能获取数据库解密密钥。\n\n' +
      '请按以下步骤重试：\n' +
      '1. 扫描时若微信被重启，等「Hook 已就绪」后再点击「登录」\n' +
      '2. 登录后打开 2～3 个聊天窗口，再重新扫描\n' +
      '3. 若仍失败，可暂时关闭杀毒/安全软件后重试',
    suggestions: ['Hook 就绪后再点登录', '登录后打开几个聊天', '暂时关闭杀毒/安全软件'],
  },
  {
    code: 'WTR-E005',
    match: (msg) =>
    ...
    userMessage:
      '解密微信数据库时失败。\n\n' +
      '请确认微信已登录，并打开几个聊天窗口后重试。\n' +
      '若仍失败，可勾选「强制重新解密」后再扫描。',
    suggestions: ['微信登录后打开几个聊天', '尝试强制重新解密', '暂时关闭杀毒/安全软件'],
  },
  {
    code: 'WTR-E006',
