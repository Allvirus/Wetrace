...
1. **选择微信数据目录** — 自动检测或手动浏览 `xwechat_files` 文件夹
2. **选择账号 → 扫描会话** — 首次可能需要几分钟；密钥与解密结果会缓存在本地，再次扫描通常更快
3. **（新版微信）配合 Hook 捕获密钥** — 工具会自动重启微信，看到「Hook 已就绪」后点击「登录」
4. **勾选会话 → 选择格式 → 导出** — 推荐勾选 HTML 格式，导出后双击 `index.html` 即可浏览
